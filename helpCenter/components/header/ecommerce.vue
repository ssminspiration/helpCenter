...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            goToLogin() {
                window.location.href = this.wwwDomain + '/login?url=' + encodeURIComponent(window.location.href);
            },
            goToRegister() {
                window.location.href = this.wwwDomain + '/reg';
            },
            goToShopCart() {
                window.open(this.wwwDomain + "/cart/redirectCart");
            },
            goToProductOrder() {
                window.open(this.wwwDomain + '/pOrder_web/productOrder/memberList');
            },
            goToServerOrder() {
                window.open(this.wwwDomain + '/sorder_web/serviceOrder/list');
            },
            goToMemberCenter() {
                window.open(this.wwwDomain + "/member/index");
            },
            goToIndex() {
                window.location.href = this.wwwDomain;
            },
            goToEntrepBackground(fId) {
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        if (res.status === 0) {
                            window.open(this.wwwDomain + "/shop_web/index?fEntrepId=" + fId);
                        }
                    }
                });
            },
            goToAddEntrep() {
                 window.open(this.wwwDomain + "/pcMemberCenter/asOne");
            },
            goToProductBargin() {
                window.open(this.wwwDomain + "/bargin_member/bargin/toUserBarginPage?type=1");
            },
            goToServerBargin() {
                window.open(this.wwwDomain + "/bargin_member/bargin/toUserBarginPage?type=2")
            },
            goToInfoCenter() {
                window.open(this.wwwDomain + "/helpMessage/memberMessList");
            },
            goToLoginOut() {
                window.location.href = this.wwwDomain + '/loginOut?url=' + window.location.href;
            },
            goToAccountInformation() {
                window.open(this.wwwDomain + "/member_web/memberCenterIndex/index");
            },
        },
        computed: {
            isLogin() {
